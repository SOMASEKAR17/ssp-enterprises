...
export default function AdminPropertiesListPage() {
  ...
  async function load() {
    setLoading(true);
    try {
      const res = await fetch('/api/properties?limit=50&adminAll=true');
  ...
  return (
    <div>
      <div className="mb-6 flex items-center justify-between">
        <h1 className="text-2xl font-bold text-brand-800">All Properties</h1>
      ...
      {loading ? (
        ...
        <div className="card p-8 text-center text-slate-500">No properties listed yet.</div>
      ) : (
          ...
          {properties.map((p) => (
            <div key={p.id} className="card flex flex-col gap-4 p-4 sm:flex-row sm:items-center">
              ...
              <div className="flex gap-2">
                <Link href={`/admin/properties/${p.id}/edit`} className="btn-outline !px-3 !py-1.5 text-xs">Edit</Link>
